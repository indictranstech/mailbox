...
class IMAPServer:
	"""Wrapper for IMAP server to pull emails."""
	...
	def get_messages(self):
		...
		try:
			...
			response, message = self.imap.uid('search',None, "UNSEEN") # search and return Uids
			...
			for i,email_meta in enumerate(pop_list):
	...
	def make_error_msg(self, msg_num, incoming_mail):
		...
		if not incoming_mail:
			try:
				# retrieve headers
				incoming_mail = Email(b'\n'.join(self.imap.top(msg_num, 5)[1]))
			except:
				pass
